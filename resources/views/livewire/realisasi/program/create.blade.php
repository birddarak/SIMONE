<tr style="background-color: rgb(218, 218, 218);" class="collapse" id="program-{{ $program->uuid }}" wire:ignore>
    ...
    <td class="p-1">
        <div class="input-group m-0">
            <input type="number" class="form-control" placeholder="Capaian" pattern="\d+" title="Input harus berupa angka" wire:model='capaian' wire:keydown.enter='store("{{ $program->uuid }}")'>
            @error('capaian')
                <span class="text-danger">{{ $message }}</span>
            @enderror
