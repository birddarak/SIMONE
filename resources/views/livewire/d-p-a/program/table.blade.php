        ...
        <table class="table custom-bordered-table">
            ...
            <tbody>
                ...
                <tr>
                    {{--
                <tr
                    class="@if (session()->has('message')) {!! session('message') == $program->uuid ? 'bg-success' : '' !!} @endif">
                    --}}
                    <td class="p-1">
                    ...
                    <td class="p-1">
                        <textarea type="text" value="{{ $program->title }}"
                            ...
                            wire:keydown.enter="updateProgram('{{ $program->uuid }}', 'title', $event.target.value)"
                            class="form-control" rows="3">{{ $program->title }}</textarea>
                    </td>
                    <td class="p-1 col-2">
                    ...
                    <td class="p-1 text-center">
                        <div class="btn-group">
                            <a href="{{ route('dpa.kegiatan', $program->uuid) }}" class="btn btn-info btn-icon mr-2">
                                <i class="ik ik-corner-down-right"></i>
                            </a>
                            @if ($program->kegiatan->count() == 0)
                            <button class="btn btn-danger btn-icon " wire:confirm='Ingin menghapus Program ini?'
